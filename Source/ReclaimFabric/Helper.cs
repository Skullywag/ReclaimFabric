    ...
    public static class Helper
    {
        public static bool IsClothes(this ThingDef def)
        {
            return !def.menuHidden &&
                  ...
                  !def.apparel.tags.NullOrEmpty() && !def.apparel.tags.Contains("Military") && !def.apparel.tags.Contains("PersonalShield");
        }
        ...
        public static bool IsAdvancedArmor(this ThingDef def)
        {
            return !def.menuHidden &&
                  ...
                  !def.apparel.tags.NullOrEmpty() && def.apparel.tags.Contains("Military") && def.apparel.tags.Contains("Spacer");
        }
        ...
        {
            var countMax = MendCountMax(thing) * 10f;
            if (countMax < 1)
            {
                return 0;
            }
            // Round to 0.1 (min) per hit point
            var perHP = countMax / thing.MaxHitPoints;
            return Mathf.Max(1, Mathf.RoundToInt(countMax / thing.MaxHitPoints)) * 0.1f;
        }

        public static int MendCountRequired(this Thing thing)
        {
            // Repair amount
            var repairAmount = thing.MaxHitPoints - (float)thing.HitPoints;
